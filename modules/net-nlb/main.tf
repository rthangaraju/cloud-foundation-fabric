...
resource "google_compute_forwarding_rule" "default" {
  ...
  ip_protocol = var.protocol # TCP, UDP, ESP, AH, SCTP, ICMP, L3_DEFAULT
  backend_service = (
    google_compute_region_backend_service.default.self_link
  )
  load_balancing_scheme = "EXTERNAL"
  ports                 = var.protocol == "L3_DEFAULT" ? null : var.ports # "nnnnn" or "nnnnn,nnnnn,nnnnn" max 5 ---ELIA: this shpould not be used if the protocol is L3_DEFAULT
  allow_global_access   = var.global_access
  labels                = var.labels
  all_ports             = var.ports == null ? true : null #---ELIA: it must be set to true when the protocol is set to L3_DEFAULT.
...
resource "google_compute_region_backend_service" "default" {
  ...
  #---ELIA: Currently this is applicable only for Internal TCP/UDP load balancing and Internal HTTP(S) load balancing
  # dynamic "subsetting" {
